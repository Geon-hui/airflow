...
import random
with DAG(
    dag_id="dags_python_operator", # DAG 이름 (Python 파일명과는 상관 없음, 일치시키는 게 좋음.)
    ...
    def select_fruit():
        fruit = ["APPLE", "BANANA", "ORANGE",'AVOADO']
        rand_int=random.randint(0,3)
        print(fruit[rand_int])

    py_t1=PythonOperator(
        task_id="py_t1", # Task ID, graph 상에서 보여지는 이름
        python_callable=select_fruit # 어떤 함수를 실행시킬지
    )
